...
onMounted(async () => {
  ...
  try {
    ...
    if (parsed && isSectionsResponse(parsed)) {
      // Filter out instrument section if user didn't select an instrument
      if (!preferences.value.instrument && parsed.sections.instrument) {
        delete parsed.sections.instrument;
      }
...
<template>
  <div class="absolute inset-0 bg-page text-white overflow-hidden">
    ...
    <div
      v-if="error"
      class="absolute inset-0 flex flex-col items-center justify-center gap-6 px-8 z-10"
    >
